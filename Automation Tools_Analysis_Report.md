...
- **각 도구의 장단점**
    - Make 🟠
        - {{formatDate(now; "YYYY-MM-DD HH:mm"; "Asia/Seoul")}} 함수를 사용하면 간단하게 한국 현재 시간을 불러올 수 있었음.
        - Trigger 설계 시, limit 수(한번에 읽을 행의 수)를 정할 수 있어 한번에 여러 새 행을 읽어야 하는 경우 실행 안정성이 높았음.
        - 동작 하지 않은 경우에도, 실행 로그 이력을 남겨 이슈 추적에 용이했음.
    - Zapier 🟢
        - 각 단계별 Test step이 있어 정상 작동을 확인하면서 진행할 수 있었음.
        - Path 기능을 사용해 설계가 다소 복잡해지면 단계별 test 데이터의 참조 이슈가 발생하여 초기 설정의 난이도가 상승함.
        - 기본 설정만으로는 한국 현재 시간이 반영되지 않아, 추가적인 Formatter모듈 또는 Code step이 필요했음.
...
설정과정의 이슈 해결 및 난이도, 연동 서비스 범위에 있어서는 비슷한 수준이었으나, 
**UI/UX 측면에서 make가 조금 더 직관적**이였고, **zapier는 추가적 code step도 필요했던 점**을 고려하면 
코딩 경험이 없는 초보자이면서 업무 자동화를 시작하는 단계에 있는 경우에는 Make를 사용하는 것이 적합할 것으로 판단된다.   

특히 이번 구현 테스트에서는 Zapier Pro를 기간제 무료 제공받아 Paths 기능을 활용하여 하나의 Zap 안에서 분기할 수 있었지만,
다음 프로젝트 구현에 Zapier 무료버전을 쓰게 된다면 path 기능 없이 여러 Zap으로 분리 구성하여 운용해야 한다는 점은 변수로 작용하여
차후 유료플랜을 활용해서 기존의 업무 자동화 설계 경험을 심화시킬 계획이 있는 경우라면 Zapier를 사용해도 무리가 없을 것으로 판단된다.  
