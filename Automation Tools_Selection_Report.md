...
- **Make** 🟠
    ...
    ![Make/Workflow 구성 화면](make_workflow.png)
    
    Make/Workflow 구성 화면
    
    ![Make/Trigger/Watch new rows/Google Sheets](make_%EC%9A%94%EC%B2%AD%EC%A0%91%EC%88%98.png)
    
    Make/Trigger/Watch new rows/Google Sheets
    
    ![Make/Action1/Send a message/Discord](make_discord.png)
    
    Make/Action1/Send a message/Discord
    
    ![Make/Action2&3/Add a row/Google Sheets  ](make_%EC%B2%98%EB%A6%AC%EB%A1%9C%EA%B7%B8.png)
    
    Make/Action2&3/Add a row/Google Sheets  
    
    - 실행 방법 : 시나리오 저장 → activate(15분마다 자동 실행) → 신규 행 추가
    - 실행 결과 :  신규 행을 정상 감지하였고, 우선순위 분기에 따라 모든 작업이 설계 의도대로 수행됨. (우선순위 높음/보통/낮음 3개 케이스 all 검증 완료)
    - 이슈 : Trigger ****기준점 설정
        - activate 모드 실행 전에, 새 행을 추가하는 경우 동작하지 않는 상황 발생 
        → ex) 새 행 추가(A7) - Activate - 새 행 추가(A8) = A7 처리 누락
        - 편집 모드에서 수동으로 시작점을 지정해서 test를 실행하고 save한 뒤, Trigger 시트를 정리하고 같은 데이터를 재사용하는 경우, activate 모드 실행 시 동작하지 않는 상황 발생
            
            → [Choose Where to start] 값을 다시 설정하면 정상 동작함을 확인함.
            
            ![Make/Choose where to start/trigger의 기준점 설정 화면](choose_where_to_start.png)
            
            Make/Choose where to start/trigger의 기준점 설정 화면
            
- **Zapier** 🟢
    
    ![Zapier/Workflow 구성 화면](%EC%9E%90%ED%94%BC%EC%96%B4_%EC%9B%8C%ED%81%AC%ED%94%8C%EB%A1%9C%EC%9A%B0.png)
    
    Zapier/Workflow 구성 화면
    
    ![Zapier/Trigger/New Spreadsheet row/Google Sheets](zapier_%EC%9A%94%EC%B2%AD%EC%A0%91%EC%88%98.png)
    
    Zapier/Trigger/New Spreadsheet row/Google Sheets
    
    ![Zapier/Action1/Send Channel message/Discord](image.png)
    
    Zapier/Action1/Send Channel message/Discord
    
    ![Zapier/Action2&3/Create spreadsheet row/Google Sheets  ](zapier_%EC%B2%98%EB%A6%AC%EB%A1%9C%EA%B7%B8.png)
    
    Zapier/Action2&3/Create spreadsheet row/Google Sheets  
...
- **비교 결과표**
    ...
    | 무료 플랜 범위 | 무료 1000 크레딧 제공
    router기능 무료 플랜 범위 내 | 무료 1000 크레딧 제공
    path기능 제한 제공(pro trial only) |
    | 실행 로그 확인 방식 | History, detail 정보 제공
    동작하지 않음 이력O | Zap runs, detail 정보 제공
    동작하지 않음 이력X |
    
    ![Make/History 화면](make_history.png)
    
    Make/History 화면
    
    ![Zapier/Zap runs 화면](zapier_run_detail.png)
